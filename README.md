...
The robot does not rely solely on PID for turning. [cite_start]Instead, it uses a **Trapezoidal-like profile** enforced by logic clamps [cite: 64-66].
...
* **Logic Flow:**
    1.  [cite_start]**Cruise Phase ($Error > 15^{\circ}$):** The controller overrides the PID output and forces `min_turn_speed` (set to 3.0 rad/s) [cite: 64-66]. This ensures fast, consistent rotation regardless of the target angle.
    ...
    3.  [cite_start]**Stop Threshold:** Control cuts off completely when error $< 0.5^{\circ}$ to prevent oscillation[cite: 67].
...
* [cite_start]**Drift Correction (Scalar):** A scalar value of `0.9935` is applied to the integration step to correct for sensor scale factor error (where the sensor reads faster/slower than reality)[cite: 59]. 
* **Drift Correction (Heading Hold):** When driving straight, a secondary PID loop (`drift_controller`) compares the current `position` against a locked `baseline_position`. [cite_start]It injects a differential speed adjustment (`correction`) to the motors to force the robot back to the centerline[cite: 71].
...
* **Filtering:** The Derivative (D) term of the PID controller utilizes a Low-Pass Filter (Bilinear Transform) to reject noise.
    * [cite_start]*Current Setting:* The cutoff frequency (`fc`) is currently set to **1.0 Hz** in `withGyroscope.ino`[cite: 38].
...
| :--- | :--- | :--- | :--- |
| **Gyro Scale** | `withGyroscope.ino` | [cite_start]`0.9935f` [cite: 59] | Multiplier to fix sensor scale error. |
| **Speed Floor** | `withGyroscope.ino` | [cite_start]`3.0f` [cite: 64] | Minimum turning speed (rad/s) when error > 15°. |
| **Turn PID** | `withGyroscope.ino` | [cite_start]`Kp=7e-3`, `Kd=1e-2` [cite: 37] | PID gains for the final approach (<15° error). |
| **Drift PID** | `withGyroscope.ino` | [cite_start]`Kp=2.5`, `Kd=0.15` [cite: 38] | Heading correction strength while driving straight. |
